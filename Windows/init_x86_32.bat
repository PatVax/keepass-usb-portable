...
:READ_PASSWORD
	...
	%sfp%x86_32\AESCrypt\aescrypt.exe -p "%pass%" -d %sfp%portable_appdata\Syncthing\config.xml.aes
...
:DECRYPT_OK
	...
	%sfp%x86_32\Syncthing\syncthing.exe -reset-database
	rem Run Syncthing. Stop by using CTRL+C command
	%sfp%x86_32\Syncthing\syncthing.exe -reset-deltas -no-browser

:ENCRYPT
	
	rem Attempt to encrypt Syncthing config with the original password
	%sfp%x86_32\AESCrypt\aescrypt.exe -p "%pass%" -e %sfp%portable_appdata\Syncthing\config.xml
...
:ENCRYPT_MANUAL
	...
	%sfp%x86_32\AESCrypt\aescrypt.exe -e %sfp%portable_appdata\Syncthing\config.xml
